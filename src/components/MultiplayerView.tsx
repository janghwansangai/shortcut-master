  ...
  return (
    <div className="w-full max-w-full flex flex-row overflow-x-auto gap-4 justify-start 2xl:justify-center items-end relative p-4 h-full">
      ...
      <div id="player-me" className="flex-1 min-w-[200px] w-full flex flex-col justify-start shrink-0 h-full relative pt-2" style={{ maxWidth: 'calc((100vh - 145px) * 0.75)' }}>
        {/* 상단 통합 메뉴바 */}
        <div className="flex items-center justify-between bg-gray-900/80 backdrop-blur-md px-3 md:px-5 py-2 md:py-3 rounded-xl border border-gray-700 shadow-xl mb-1 z-10 relative w-full gap-2">
          {/* 나의 아이디 및 미션 영역 */}
          <div className="flex items-center flex-shrink-0 gap-2 md:gap-3">
            <span className="text-white font-black text-lg md:text-xl lg:text-2xl bg-blue-600/80 px-3 md:px-4 py-1 rounded border border-blue-400 shrink-0 hidden sm:inline">
              {playerName}(나)
            </span>
            <span className="text-white font-black text-base bg-blue-600/80 px-2 py-1 rounded border border-blue-400 shrink-0 sm:hidden">
              나
            </span>
            {currentMission ? (
              <div className="flex items-center bg-yellow-500/20 px-2 md:px-3 py-1 rounded-lg border border-yellow-500/50 shadow-[0_0_10px_rgba(234,179,8,0.2)]">
                <span className="text-base md:text-lg lg:text-xl font-black text-yellow-400 mr-1 md:mr-2 animate-pulse hidden sm:inline">🔥 미션:</span>
                <span className="text-base md:text-lg lg:text-xl font-black text-yellow-400 mr-1 md:mr-2 animate-pulse sm:hidden">🔥</span>
                <span className="text-lg md:text-xl lg:text-2xl font-bold text-white tracking-widest bg-gray-900/50 px-2 md:px-3 py-1 rounded-md border border-gray-700">
